import { Service, Package, PricingComponent, Question, QuestionOption, ValidationRule, Multiplier, GlobalSettings } from "@prisma/client";
import { Decimal } from "@prisma/client/runtime/library";
import { EstimateCalculateInput } from "../schemas/estimate.schemas";
import { TotalCalculationResult, ServiceCostBreakdown, CostBreakdownItem } from "./pricing.types";
import { calculateTimeline } from "./pricing.helpers";
import { ApiError } from "../middleware/error.middleware";

// Full dynamic representation matching active database joins
...
export class PricingEngine {
  static calculate(
    input: EstimateCalculateInput,
    services: FullService[],
    dbMultipliers: Multiplier[],
    settings: GlobalSettings
  ): TotalCalculationResult {
    const servicesBreakdown: ServiceCostBreakdown[] = [];
    let totalBaseCost = new Decimal(0);
    let totalCalculatedCost = new Decimal(0);

    for (const serviceInput of input.services) {
      const service = services.find((s) => s.id === serviceInput.serviceId);
      if (!service || service.status !== "ACTIVE") {
        throw new ApiError(404, "NOT_FOUND", `Active service ${serviceInput.serviceId} not found`);
      }
      ...
      if (selectedPackageId) {
        ...
        if (!pkg || pkg.status !== "ACTIVE") {
          throw new ApiError(400, "BAD_REQUEST", `Active package ${selectedPackageId} does not belong to service ${service.id}`);
        }
        basePrice = new Decimal(pkg.price);
      ...
      let flatAddons = new Decimal(0);
      let multiplierProduct = new Decimal(1.0);
      ...
      for (const question of allQuestions) {
        ...
        if (question.conditionalParentId) {
          ...
          if (!isVisible) continue; // Skip question validation and calculation if not visible
        }

        // Required question validation
        if (question.isRequired && (selectedValue === undefined || selectedValue === null || selectedValue === "")) {
          throw new ApiError(400, "BAD_REQUEST", `Answer to required question ${question.id} is missing`);
        }
        ...
        if (question.validationRule) {
          ...
          if (["counter", "number"].includes(question.type)) {
            ...
            if (isNaN(valNum)) {
              throw new ApiError(400, "BAD_REQUEST", `Value for ${question.id} must be a number`);
            }
            if (rule.min !== null && rule.min !== undefined && valNum < rule.min) {
              throw new ApiError(400, "BAD_REQUEST", `Value for ${question.id} cannot be less than ${rule.min}`);
            }
            if (rule.max !== null && rule.max !== undefined && valNum > rule.max) {
              throw new ApiError(400, "BAD_REQUEST", `Value for ${question.id} cannot be more than ${rule.max}`);
            }
          }
          if (question.type === "TEXT" && typeof selectedValue === "string") {
            ...
            if (rule.min !== null && rule.min !== undefined && length < rule.min) {
              throw new ApiError(400, "BAD_REQUEST", `Length of text for ${question.id} cannot be less than ${rule.min}`);
            }
            if (rule.max !== null && rule.max !== undefined && length > rule.max) {
              throw new ApiError(400, "BAD_REQUEST", `Length of text for ${question.id} cannot be more than ${rule.max}`);
            }
          }
        }
        ...
        if (["RADIO", "CHECKBOX", "SELECT"].includes(question.type) && question.options) {
          ...
          for (const optVal of submittedArray) {
            ...
            if (!option) {
              throw new ApiError(400, "BAD_REQUEST", `Invalid option value '${optVal}' for question ${question.id}`);
            }
            const mod = new Decimal(option.priceModifier);
        ...
        if (question.type === "TOGGLE" && selectedValue === true) {
          const mod = new Decimal(question.priceModifier);
        ...
        if (["COUNTER", "NUMBER"].includes(question.type)) {
          ...
          if (val > 0 && !mod.isZero()) {
            if (type === "FLAT") {
              ...
              const factor = new Decimal(1.0).add(mod.sub(1.0).mul(val));
      ...
      for (const compInput of selectedComponentsInput) {
        ...
        if (!comp || comp.status !== "ACTIVE") {
          throw new ApiError(404, "NOT_FOUND", `Active pricing component ${compInput.pricingComponentId} not found in service ${service.id}`);
        }
      ...
      if (selectedPackageId) {
        const pkg = service.packages.find((p) => p.id === selectedPackageId);
        if (pkg) baselineTimeline = pkg.timeline;
      }
      ...
      const timelineResult = calculateTimeline(
        baselineTimeline,
        activeAddonsCount,
        input.multipliers.complexity || "simple",
        input.multipliers.urgency || "normal"
      );
    ...
    const compMultiplierOpt = dbMultipliers.find((m) => m.category === "COMPLEXITY" && m.id === (input.multipliers.complexity || "simple")) || { value: new Decimal(1.0) };
    const urgMultiplierOpt = dbMultipliers.find((m) => m.category === "URGENCY" && m.id === (input.multipliers.urgency || "normal")) || { value: new Decimal(1.0) };
    const qualMultiplierOpt = dbMultipliers.find((m) => m.category === "QUALITY" && m.id === (input.multipliers.quality || "standard")) || { value: new Decimal(1.0) };

    const projectMultiplier = new Decimal(compMultiplierOpt.value).mul(new Decimal(urgMultiplierOpt.value)).mul(new Decimal(qualMultiplierOpt.value));

    let oneTimeSubtotal = new Decimal(0);
    let monthlySubtotal = new Decimal(0);

    servicesBreakdown.forEach((srv) => {
      let srvOneTime = new Decimal(0);
      let srvMonthly = new Decimal(0);
    ...
    let oneTimeDiscount = new Decimal(0);
    let monthlyDiscount = new Decimal(0);
    const discountRate = new Decimal(settings.discountRate);
    ...
    let oneTimeTax = new Decimal(0);
    let monthlyTax = new Decimal(0);
    const taxRate = new Decimal(settings.taxRate);
    if (taxRate.gt(0)) {
    ...
    const minCost = new Decimal(settings.minimumCost);
    const maxCost = new Decimal(settings.maximumCost);

    if (finalCost.lt(minCost)) finalCost = minCost;
    if (finalCost.gt(maxCost)) finalCost = maxCost;
